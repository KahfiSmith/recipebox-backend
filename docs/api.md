...
- Verification emails include an 8-digit numeric verification code in the email body. If `API_PUBLIC_BASE_URL` is configured, the email also includes a backend verification link that can be opened directly and is handled by `GET /api/v1/auth/verify-email/confirm`. When `API_PUBLIC_BASE_URL` is not set, the service falls back to the existing frontend verification link generated from `FRONTEND_BASE_URL`.
